...
# 批量模式：把 anime 目录里的 4 个嘴型视频抽帧到 datamouth
BATCH_MODE = True
INPUT_DIR = BASE_DIR / "anime"
OUTPUT_ROOT = BASE_DIR / "datamouth"

MOUTH_VIDEO_MAP = {
    "small.mp4": "mouth_small_open",
    "big.mp4": "mouth_big_open",
    "wide.mp4": "mouth_wide",
    "oshape.mp4": "mouth_round",
}

# 单文件模式（保留兼容）
VIDEO_PATH = str(INPUT_DIR / "small.mp4")
OUTPUT_DIR = str(OUTPUT_ROOT / "mouth_small_open")

TARGET_SIZE = (284, 240)              # 目标尺寸 (宽, 高)
TARGET_FRAME_COUNT = 15               # 每个嘴型输出 15 帧
...
def main():
    ...
    print("=" * 60)
    print(f"  目标尺寸: {TARGET_SIZE[0]}x{TARGET_SIZE[1]}")
    print(f"  目标帧数: {TARGET_FRAME_COUNT} 帧")
    print(f"  JPEG质量: {JPEG_QUALITY}")
    print()

    if BATCH_MODE:
        print(f"批量输入目录: {INPUT_DIR}")
        print(f"批量输出目录: {OUTPUT_ROOT}")
        print(f"嘴型映射: {MOUTH_VIDEO_MAP}")
        print("=" * 60)
        print()

        OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
        for video_name, output_name in MOUTH_VIDEO_MAP.items():
            video_path = INPUT_DIR / video_name
            output_dir = OUTPUT_ROOT / output_name
            print(f"\n--- 处理 {video_name} -> {output_name} ---")
